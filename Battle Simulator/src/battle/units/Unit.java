...
public class Unit {
	...
	private void attack() {
		...
		if (team == 0) {
			...
			for (Unit u : BattleSim.team2) {
				if (type.equals("Ranged")) {
					...
					int power = 0;
					power = (int)(u.melee_power * ((double)u.health/(double)u.fullhealth));
					...
					if (u.type.equals("Ranged")) {
						...
						r = ((double)(u.ranged_power) * ((double)u.health/(double)u.fullhealth))/(double)fullhealth;
						r = r/0.1;
						...
						r = (((double)melee_power * ((double)health/(double)fullhealth)))/((double)(u.melee_power * ((double)u.health/(double)u.fullhealth)));
		...
		if (type.equals("Melee")) {
			//if possible defender is a melee unit, find ratio of current units melee power to possible defender's melee power.
			double r = ((double)(melee_power * ((double)health/(double)fullhealth))/(double)(defender.melee_power * ((double)defender.health/(double)defender.fullhealth)));
			...
			double y_min_d = (4/5) * ranged_power * ((double)health/(double)fullhealth);
			double y_max_d = (6/5) * ranged_power * ((double)health/(double)fullhealth);
